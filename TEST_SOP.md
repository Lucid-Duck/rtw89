...
| D-Link IP | 192.168.99.110 |
| Alfa Interface | wlp0s13f0u2i3 (DO NOT USE FOR TESTS) |
| Alfa Driver | mt7921u |
| Alfa IP | 192.168.99.231 |
| iperf3 Server | 192.168.99.250 (Windows 11 PC) |
...
1. **Interface Check**: `ip -br addr show wlp0s20f0u6` - must show IP 192.168.99.110
2. **Driver Check**: Confirm rtw89_8852au_git is the driver
3. **Route Check**: `ip route get 192.168.99.250 from 192.168.99.110` - must show `dev wlp0s20f0u6`
...
sudo ip route replace 192.168.99.250 dev wlp0s20f0u6 src 192.168.99.110
```
...
# Verify:
ip route get 192.168.99.250 from 192.168.99.110
